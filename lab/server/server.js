...
// This is intended to protect the user's api key. The api key is a unique access code provided to the user which allows access to api methods.
// In this case, the api key is provided by Google.
...
app.get('/api/v1/books/find', (req, res) => {
  ...
  // These lines append information (if provided) to the query.
  let query = ''
  ...
  // Superagent is a HTTP request library. It is meant to simplify ajax requests. Here we are using it to add parameters to the request. Axios, Got, and Request are a few other libraries that could be used here.
  superagent.get(url)
    .query({'q': query})
    .query({'key': API_KEY})
    .then(response => response.body.items.map((book, idx) => {
      ...
      // Destructuring allows us to declare several variables at once and give them values from an object that has properties of the same names.
      let { title, authors, industryIdentifiers, imageLinks, description } = book.volumeInfo;

      // DONE: What is the purpose of the following placeholder image?
      // This image will be displayed should the query return a result without a valid image url.
      let placeholderImage = 'http://www.newyorkpaddy.com/images/covers/NoCoverAvailable.jpg';

      // DONE: Explain how ternary operators are being used below.
      // These ternary operators are being used to provide alternative values should the query result contain null or empty values.
      return {
...
// This query uses an additional parameter. The 'isbn' is the parameter provided from the Book.findOne method.
