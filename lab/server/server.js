...
// The API_KEY comes from Google, and serves as a password to prevent malicious activity.
...
app.get( '/api/v1/books/find', ( req, res ) => {
  ...
  // These if statements are checking the request for the corresponding values to add to the query. If they are found, they are added.
  let query = '';
  ...
  // SuperAgent is an ajax API designed to work with Node.js.
  superagent.get( url )
    .query( { 'q': query } )
    .query( { 'key': API_KEY } )
    .then( response => response.body.items.map( ( book, idx ) => {
      ...
      // Destructuring is a method of assigning multiple properties to an object in shorthand.
      let { title, authors, industryIdentifiers, imageLinks, description } = book.volumeInfo;

      // COMMENT: What is the purpose of the following placeholder image?
      // This will be the image displayed if the image_url doesn't return a valid image.
      let placeholderImage = 'http://www.newyorkpaddy.com/images/covers/NoCoverAvailable.jpg';

      // COMMENT: Explain how ternary operators are being used below.
      // For each property, the ternary operator checks for exisiting information, and if none is available, sets a placeholder.
      return {
...
// This will only return entries from the database matching a particular ISBN. The ':isnb' in the path is a route parameter which makes a more specific end point for the get request.
