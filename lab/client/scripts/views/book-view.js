...
( function ( module ) {
  ...
  // This function displays the .search-view class, i.e. the search bar, and adds and event listener to the submit button.
  bookView.initSearchFormPage = function () {
    ...
    $( '#search-form' ).on( 'submit', function ( event ) {
      ...
      // It refers to each field of the search form, with each attribute corresonding to the id of that form, and the value corresponding to the user's input in that field. For each field that is left blank, the object will be replace that property with an empty string.
      let book = {
        title: event.target.title.value || '',
        author: event.target.author.value || '',
        isbn: event.target.isbn.value || '',
      ...
      // These values are being set to empty strings in order to clear the form for the next search.
  ...
  // COMMENT: What is the purpose of this method?
  // It empties the previous search results, and then displays the new ones.
  bookView.initSearchResultsPage = function () {
    app.showOnly( '.search-results' );
    $( '#search-list' ).empty();

    // COMMENT: Explain how the .forEach() method is being used below.
    // This function appends the rendered books in Book.all to the search-list section.
    module.Book.all.forEach( book => $( '#search-list' ).append( book.toHtml() ) );
    $( '.detail-button a' ).text( 'Add to list' ).attr( 'href', '/' );
    $( '.detail-button' ).on( 'click', function ( e ) {
      // COMMENT: Explain the following line of code.
      // This function traverses the DOM to access the ISBN number of a given book.
